...
export async function POST(req: Request, routeCtx: RouteContext) {
  ...
  try {
    ...
    const { result } = await commandBus.execute('anter_configurator.submission.request_changes', {
...
export const openApi: OpenApiRouteDoc = {
  ...
  summary: 'Request changes on a submission',
  methods: {
    POST: {
      summary: 'technical_review → revision_requested; the revision itself stays immutable, the partner must branch a new one',
      requestBody: { contentType: 'application/json', schema: bodySchema },
      responses: [{ status: 200, description: 'Requested', schema: z.object({ item: z.unknown() }) }],
      errors: [
